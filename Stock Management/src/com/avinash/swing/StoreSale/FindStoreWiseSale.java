package com.avinash.swing.StoreSale;
import com.avinash.jdbc.daos.StoreSalesDao;
import com.avinash.jdbc.pojos.StoreSales;
import com.avinash.jdbc.utlities.*;
import javax.swing.*;
import javax.swing.border.BevelBorder;

import java.awt.BorderLayout;
import java.awt.Color;
import java.awt.FlowLayout;
import java.awt.Font;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import java.text.SimpleDateFormat;

public class FindStoreWiseSale extends JPanel implements ActionListener {
	JLabel lb1, lb2, lb3;
	JFormattedTextField tf1, tf2;
	JTable table;
	JPanel northPanel, centerPanel,south;
	
	public FindStoreWiseSale() {
		setBackground(Color.white);
		setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
		setLayout(new BorderLayout());
		northPanel = new JPanel(new FlowLayout());
		south = new JPanel(new FlowLayout());
		
		lb1 = new JLabel();
		lb1.setText("Date");
		lb1.setFont(new Font("Serif", Font.BOLD, 15));
		lb1.setForeground(Color.black);;
		northPanel.add(lb1);
		
		tf1 = new JFormattedTextField(new SimpleDateFormat("dd-MM-yyyy").format(new java.util.Date()));
		tf1.setColumns(18);
		tf1.setBackground(Color.white);
		tf1.setCaretColor(Color.black);
		tf1.setSelectedTextColor(Color.red);
		tf1.setFont(new Font("Times New Roman", Font.BOLD, 12));
		northPanel.add(tf1);
		
		lb2 = new JLabel();
		lb2.setText("Store Id");
		lb2.setFont(new Font("Serif", Font.BOLD, 15));
		lb2.setForeground(Color.black);;
		northPanel.add(lb2);
		
		tf2 = new JFormattedTextField(new String());
		tf2.setColumns(18);
		tf2.setBackground(Color.white);
		tf2.setCaretColor(Color.black);
		tf2.setSelectedTextColor(Color.red);
		tf2.setFont(new Font("Times New Roman", Font.BOLD, 12));
		tf2.addActionListener(this);
		northPanel.add(tf2);
		
		lb3 = new JLabel();

		
		northPanel.setOpaque(false);
		
		add(northPanel, BorderLayout.NORTH);
		
		DateWiseStoreSalesTableModel med = new DateWiseStoreSalesTableModel(new java.util.Date(),0);
		table = new JTable(med);
		table.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED));
		
		add(new JScrollPane(table), BorderLayout.CENTER);
		add(south, BorderLayout.SOUTH);
	}

	@Override
	public void actionPerformed(ActionEvent e) {
		// TODO Auto-generated method stub
		
		DateWiseStoreSalesTableModel mt = new DateWiseStoreSalesTableModel(DateUtils.convertDate(tf1.getText().trim()),Integer.parseInt(tf2.getText().trim()));
	    table.setModel(mt);
	    
	    StoreSalesDao storeSaleDao  = new StoreSalesDao();
		StoreSales ss =  storeSaleDao.totalCollection(DateUtils.convertDate(tf1.getText().trim()));
		
		lb3.setText("");
		lb3.setText("Amount   "+ss.getAmount()+"   \nDiscount   "+ss.getDiscount()+"  \nActual Collected   "+ss.getTotalPayable());
		lb3.setFont(new Font("Serif", Font.BOLD, 15));
		lb3.setForeground(Color.black);
		south.add(lb3);
		
		
		south.invalidate();
		south.updateUI();
		
	}
	
	public static void main(String args[]) {
		JFrame f = new JFrame("Find Store Sales");
		f.setContentPane(new FindStoreWiseSale());
		f.setBounds(1000, 1000, 1100, 730);
		f.setVisible(true);
		f.setLocationRelativeTo(null);
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	}

}
